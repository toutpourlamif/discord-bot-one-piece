...
Principe : on connaît la **fraction du voyage déjà parcourue** via `(currentBucket - travel_started_bucket) / (travel_eta_bucket - travel_started_bucket)`. On recalcule la durée totale avec les nouvelles conditions, et on n'applique le malus qu'à la part **restante** :

```
nouvelleEtaBucket = currentBucket + round(nouvelleDuréeBuckets × (1 - progressActuel))
```
...
| Event d'arrivée                           | Une fois quand `currentBucket >= player.travel_eta_bucket`, automatique. |
